...
import { merge, noop, sprintf, tryRead, write } from './helpers';
import { ExtendedInfo, TputOptions, TerminalInfo } from './define';
export { sprintf, tryRead } from './helpers';
...
export class Tput {
  ...
  debug?: any;
  padding?: any;
  extended: TputOptions['extended'];
  printf: TputOptions['printf'];
  termcap?: any;
  error: null | Error;
  terminfoPrefix?: any;
  terminfoFile?: any;
  termcapFile?: any;
  _termcap: ReturnType<typeof this._tryCap>;
  all: TerminalInfo['all'];
  _terminfo?: TerminalInfo;
  info?: TerminalInfo;
  methods: TerminalInfo['methods'];
  bools: TerminalInfo['bools'];
  numbers: TerminalInfo['numbers'];
  strings: TerminalInfo['strings'];
  features: TerminalInfo['features'];

  // Infers
  brokenACS: any;
  constructor(opts: string | TputOptions = {}) {
    ...
    this.printf = opts.printf;
    this.termcap = opts.termcap;
    this.error = null;

    this.terminfoPrefix = opts.terminfoPrefix;
    this.terminfoFile = opts.terminfoFile;
    this.termcapFile = opts.termcapFile;

    if (opts.terminal || opts.term) {
      this.setup();
    }
  }
  ...
  static _prefix = Tput.prototype._prefix;
  ...
  private _tprefix(
    ...
    let sfile: undefined | string = undefined;
    ...
    const find = function (word: string): string | undefined {
    ...
    if (!term) {
      ...
      try {
        const dirs = fs.readdirSync(prefix).filter(function (file) {
  ...
  static _tprefix = Tput.prototype._tprefix;
  ...
  parseTerminfo(data: Buffer, file: string) {
    ...
    Object.keys(info.strings).forEach(function (key) {
      ...
      info.strings[key] = parseInt(data.toString('ascii', s, j));
    });
  ...
  parseExtended(data: Buffer) {
    ...
    // Strings Section
    const _strings: (number | '')[] = []; // TODO Uncomfortable Implicit Conversion here
    l = i + h.strCount * 2;
    for (; i < l; i += 2) {
      if (data[i + 1] === 0xff && data[i] === 0xff) {
        _strings.push(-1);
    ...
    _strings.forEach(function (offset, k) {
      ...
      _strings[k] = parseInt(data.toString('ascii', s, j));
    });
  ...
  compile(info: TerminalInfo) {
    ...
    (['bools', 'numbers', 'strings'] as const).forEach((type) => {
      Object.keys(info[type]).forEach((key) => {
        info.all![key] = info[type][key];
        info.methods![key] = this._compile(info, key, info.all![key]);
      });
    });
  ...
  inject(info: TerminalInfo) {
    //TODO Not sure info's type
    const methods = info.methods || info;
    ...
    if (!info.features) {
      throw new Error("There's should be a [features Object]");
    }
    Object.keys(info.features).forEach((key) => {
      if (key === 'padding') {
        if (!info.features?.padding && this.options.padding !== true) {
          this.padding = false;
        }
        return;
      }
      this[key] = info.features?.[key];
    });
  }
  // See:
  // ~/ncurses/ncurses/tinfo/lib_tparm.c
  // ~/ncurses/ncurses/tinfo/comp_scan.c
  private _compile(info: TerminalInfo, key, str) {
    var v;
  ...
  _print(
    code: string,
    print?: (data: Uint8Array | string) => boolean,
    done?: () => string,
  ) {
    ...
    (function next() {
      ...
      let amount = +padding[1];
      let suffix = padding[2];
  ...
  static print(...args: any[]) {
    ...
    return Tput.prototype._print.apply(fake, [...args]);
  }
  ...
  readTermcap(term?: string) {
    let terms: ReturnType<typeof this._tryCap>;
    let root, paths;
    ...
    if (~term.indexOf(path.sep) && (terms = this._tryCap(path.resolve(term)))) {
      const term_ = path.basename(term).split('.')[0];
      if (process.env.TERM && terms[process.env.TERM]) {
  ...
  _tryCap(file: string | string[], term?: string): undefined | typeof terms {
    if (!file) return;

    let data: string | typeof terms | undefined;
    ...
    const terms = this.parseTermcap(data, file);
  ...
  translateTermcap(info) {
    ...
    ['bools', 'numbers', 'strings'].forEach((key) => {
      ...
      Object.keys(info[key]).forEach((cap) => {
        if (key === 'strings') {
          info.strings[cap] = this._captoinfo(cap, info.strings[cap], 1);
        }
        if (map[cap]) {
   ...
   */
  private _captoinfo(cap, s, parameterized) {
    var capstart;
  ...
  getAll() {
    ...
    const list = asort(fs.readdirSync(dir));
    const infos: string[] = [];
    list.forEach(function (letter) {
      const terms = asort(fs.readdirSync(path.resolve(dir, letter)));
      infos.push(...terms);
    });
    function asort(obj: Array<string>) {
      return obj.sort(function (a, b) {
        const da = a.toLowerCase().charCodeAt(0);
        const db = b.toLowerCase().charCodeAt(0);
        return da - db;
      });
    }
    return infos;
  ...
  compileAll(start) {
    const all: Record<string, TerminalInfo> = {};
    ...
    this.getAll().forEach((name) => {
      ...
      all[name] = this.compileTerminfo(name);
    });
   ...
   */
  detectFeatures(info: TerminalInfo) {
  ...
  detectPCRomSet(info: TerminalInfo) {
    var s = info.strings;
    if (
  ...
  GetConsoleCP() {
    ...
    if (
      !process.env.NCURSES_NO_WINDOWS_UNICODE ||
      +process.env.NCURSES_NO_WINDOWS_UNICODE !== 1
    ) {
      return 65001;
    }
  ...
  static alias = (function () {
    ...
    result.dest_tabs_magic_smso.push('teleray_glitch');

    // Numbers
    result.micro_col_size.push('micro_char_size');
    return result;
  })();
