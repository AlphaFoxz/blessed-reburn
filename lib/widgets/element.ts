...
class Element extends Node {
  ...
  constructor(options) {
    ...
    if (options.scrollable && !this._ignore && this.type !== 'scrollable-box') {
      ...
      Object.getOwnPropertyNames(ScrollableBox.prototype).forEach((key) => {
        if (key === 'type') return;
  ...
  sattr(style: Record<string, any>, fg?: Function, bg?: Function) {
    var bold = style.bold,
  ...
  setContent(content, noClear: boolean, noTags?: boolean) {
    if (!noClear) {
      this.clearPos();
    }
  ...
  _parseAttr(lines) {
    ...
    for (let j = 0; j < lines.length; j++) {
      ...
      for (let i = 0; i < line.length; i++) {
        if (line[i] === '\x1b') {
          let c = /^\x1b\[[\d;]*m/.exec(line.substring(i));
          if (c) {
  ...
  _wrapContent(content: string, width: number) {
  ...
  enableDrag(verify) {
    ...
    this.onScreenEvent(
      ...
      (this._dragM = (data) => {
        ...
        var ox = this._drag?.x,
          oy = this._drag?.y,
          px = this.parent.aleft,
          py = this.parent.atop,
          x = data.x - px - ox!, // FIXME The `ox` perhaps be undefined
          y = data.y - py - oy!;
  ...
  setLabel(options) {
    ...
    var reposition = () => {
      this._label!.rtop = (this.childBase || 0) - this.itop; //FIXME perhaps be undefined
      if (!this.screen.autoPadding) {
  ...
  removeLabel() {
    ...
    this.removeListener('scroll', this._labelScroll!); // FIXME Perhaps be undefined
  ...
  set rtop(val: number) {
    ...
    if (/^\d+$/.test(val.toString())) val = +val;
    this.emit('move');
    this.clearPos();
    this.position.top = val;
    // return (this.position.top = val);
  ...
  _getShrinkContent(xi, xl, yi, yl, get?: any /* Unused variable */) {
  ...
  _getCoords(get?: boolean, noscroll?: boolean) {
    if (this.hidden) return;
  ...
  render() {
    ...
    if (this.scrollbar) {
      ...
      i = Math.max(this._clines.length, this._scrollBottom()); // FIXME There's no `_scrollBottom` in `class Element`
    }
  ...
  screenshot(xi, xl, yi, yl) {
    if (!this.lpos) {
      throw new Error('Element has not been rendered.');
    }
