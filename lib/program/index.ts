...
import { Key, ProgramOptions } from './define';
...
class Program extends EventEmitter {
  ...
  input: Required<ProgramOptions>['input'];
  output: Required<ProgramOptions>['output'];
  _logger: fs.WriteStream;
  zero: ProgramOptions['zero'];
  useBuffer: ProgramOptions['buffer'];
  x: number;
  y: number;
  savedX: number;
  savedY: number;
  ...
  _newHandler?: (...args: any[]) => void;
  // removeListener: (name: string, fn: Function) => any;
  destroyed: boolean;
  _lastButton?: string;
  gpm?: GpmClient;
  ret?: boolean;
  ...
  // Infers
  hideCursor_old?: Function;
  showCursor_old?: Function;
  constructor(
    param1?: ProgramOptions | ProgramOptions['input'],
    param2?: ProgramOptions['output'],
  ) {
    super();
    const opts: ProgramOptions = {} as ProgramOptions;
    Program.bind(this);
    // EventEmitter.call(this);

    if (!param1 || Object.getPrototypeOf(opts) !== Object.prototype) {
      opts.input = param1 as ProgramOptions['input'];
      opts.output = param2 as ProgramOptions['output'];
    }
    ...
    opts.log = opts.log || opts.dump;
    if (opts.log) {
      this._logger = fs.createWriteStream(opts.log);
      if (opts.dump) this.setupDump();
    }

    this.zero = opts.zero !== false;
    this.useBuffer = opts.buffer;
    ...
    this.isXFCE = /xfce/i.test(process.env.COLORTERM!); //TODO
    ...
    this.isRxvt = /rxvt/i.test(process.env.COLORTERM!); //TODO
    this.isXterm = false;
  ...
  _log(pre: 'LOG' | 'DEBUG' | 'IN' | 'OUT', msg: string) {
      ...
      return data.replace(/[\0\x80\x1b-\x1f\x7f\x01-\x1a]/g, function (ch) {
        switch (ch) {
          ...
          case '\x7f':
            ...
            break;
          default:
            const ch2int = ch.charCodeAt(0);
            // From ('A' - 64) to ('Z' - 64).
            if (ch2int >= 1 && ch2int <= 26) {
              ch = String.fromCharCode(ch2int + 64);
            } else {
              return String.fromCharCode(ch2int);
            }
            break;
        }
        return '^' + ch;
  ...
  listen() {
    ...
    //   this.manipulateWindow(21, (err, data) => {
    //     if (err) return;
  ...
  _listenInput() {
    ...
    this.input.on(
      'keypress',
      (this.input._keypressHandler = (ch, key: Key) => {
        key = key || { ch: ch };
    ...
    this.input.on(
      ...
      (this.input._dataHandler = (data) => {
        Program.instances.forEach((program) => {
          if (program.input !== this.input) {
            return;
          }
          const p = program as Program & EventEmitter;
          p.emit('data', data);
        });
      }),
    );
  ...
  destroy() {
    ...
    if (~index) {
      ...
      if (typeof this.input._blessedInput === 'number') {
        this.input._blessedInput--;
      }
      if (typeof this.output._blessedOutput === 'number') {
        this.output._blessedOutput--;
      }
  ...
  unkey(key: string | string[], listener: (...args: any[]) => void) {
    if (typeof key === 'string') {
      key = key.split(/\s*,\s*/);
    }
    key.forEach((key) => {
  ...
  enableGpm() {
    ...
    this.on('btndown', (btn, modifier, x, y) => {
      x--, y--;
  ...
  _bindResponse(s: Buffer | string) {
    type Out = {
      ...
      firmwareVersion?: any;
      romCartridgeRegistrationNumber?: any;
      deviceAttributes?: Out;
      status?: string | { x: number; y: number; page?: number };
      x?: number;
      y?: number;
      page?: number;
      deviceStatus?: string;
      ...
      windowTitle?: string;
      error?: string;
      cursor?: string | { x: number; y: number; page?: number };
      state?: string;
      windowState?: string;
    ...
    let parts: RegExpExecArray | any[] | null =
      /^\x1b\[(\?|>)(\d*(?:;\d*)*)c/.exec(s);
    ...
    if (parts) {
      ...
      if (parts && parts[1] === '?') {
        ...
        out.firmwareVersion = parts && parts[1];
        out.romCartridgeRegistrationNumber = parts && parts[2];
      }
  ...
  response(name, text, callback, noBypass?: boolean) {
  ...
  lrestoreCursor(key: string, hide) {
    var pos;
    key = key || 'local';
  ...
  charset(val, level?: number | string) {
    level = level || 0;
  ...
  as = this.smacs;
  enter_alt_charset_mode = this.as;
  rmacs() {
    return this.charset('ascii');
  }
  ae = this.rmacs;
  exit_alt_charset_mode = this.ae;
  // ESC N
  ...
  setMode(...args: any[]) {
    var param = slice.call(args).join(';');
  ...
  vi = this.civis;
  cursor_invisible = this.vi;
  dectcemh = this.cursor_invisible;
  normalBuffer() {
    this.isAlt = false;
  ...
  enableMouse() {
    if (process.env.BLESSED_FORCE_MODES) {
      ...
      var options: Record<string, boolean> = {};
  ...
  // Set Mouse
  setMouse(opt: Record<string, boolean>, enable?: boolean) {
  ...
  req_mouse_pos = this.reqmp;
  decrqlp = this.req_mouse_pos;
  // CSI P m SP }
  ...
  pause(callback?: Function) {
    ...
    const write = this.output.write;
    (this.output as any).write = function () {}; // TODO: Hack code
    if (this.input.setRawMode) {
      this.input.setRawMode(false);
    }
    this.input.pause();
...
function merge(out, ...others: any[]) {
  slice.call([out, ...others], 1).forEach(function (obj) {
